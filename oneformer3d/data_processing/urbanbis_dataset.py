...
class URBANBISDataset(Det3DDataset):
    r"""UrbanBIS Dataset for Detection Task.
    ...
    """
    METAINFO = {
        'classes': ('terrain', 'vegetation', 'water', 'bridge', 'vehicle', 'boat', 'building'),
        'seg_valid_class_ids': tuple(range(7)),
        'seg_all_class_ids': tuple(range(8))
    }
    ...
    def parse_data_info(self, info: dict) -> dict:
        ...
        # Add multi-view image handling
        if 'images' in info:
            image_info = info['images']
            if 'paths' in image_info and 'poses' in image_info:
                info['images'] = {
                    'paths': [osp.join(self.data_prefix['images'], p) for p in image_info['paths']],
                    'poses': image_info['poses']
                }
...
# 该类专门用于处理单一区域的 UrbanBIS 数据
class _URBANBISSegDataset(Seg3DDataset):
    r"""UrbanBIS Dataset for Semantic Segmentation Task.
    ...
    def __init__(self,
                 ...
                 metainfo: Optional[dict] = None,
                 data_prefix: dict = dict(
                     pts='', pts_instance_mask='', pts_semantic_mask='', images=''),
                 pipeline: List[Union[dict, Callable]] = [],
...
# 该类专门用于处理多个区域的 UrbanBIS 数据
@DATASETS.register_module()
class URBANBISSegDataset(_URBANBISSegDataset):
    r"""UrbanBIS Dataset for Semantic Segmentation Task.
    ...
    def __init__(self,
        ...
        # Add multi-view specific validation
    ...
    def _validate_modality(self):
        if not self.modality.get('use_camera', False):
            mmengine.print_log('Multi-view dataset requires use_camera=True', level='warning')
